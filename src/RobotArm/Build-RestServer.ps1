...
function New-ZipFromFolderContents {
    ...
    if ($null -eq $itemsToZip) {
        ...
        Write-Verbose "Successfully created empty archive at '$resolvedDestinationPath'."
        return
    }
...
try
{
    ...
    if ($UploadToCar)
    {
        Write-Host "Zipping contents of NetworkController..."
        $releasePath = Join-Path -Path $currentPath -ChildPath "NetworkController\bin\Release\net8.0\linux-arm64\publish\"
        $zipPath = Join-Path -Path $currentPath -ChildPath "NetworkController.zip"
        ...
        Write-Verbose "Reading file content from '$zipPath'..."
        $fileBytes = [System.IO.File]::ReadAllBytes($zipPath)
        $fileStreamLength = $fileBytes.Length
        Write-Verbose "File size is $fileStreamLength bytes."
        
        $ApiUrl = "http://192.168.200.54:5000/api/v1/software/restserver"
        $headers = @{

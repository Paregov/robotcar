...
def find_newest_version_dll(base_folder):
    ...
    if newest_version_path:
        dll_path = os.path.join(newest_version_path, "NetworkController.dll")
        return dll_path
    else:
        return None
...
if __name__ == "__main__":
    ...
    base_folder_to_search = "/home/paregov/robotcar/restserver"
    returned_path = find_newest_version_dll(base_folder_to_search)

    if returned_path:
        print(f"The newest version's NetworkController.dll path is: {returned_path}")
